...
$("#iltbtnx").click(function(){
    $.ajax({
        type:'POST',
        url:'ajax/sv_servis_talep.html',
        data:$('#iltfrmx').serialize(),
		beforeSend: function(msg) {
        $("#iltbtnx").val('Lütfen Bekleyin...').attr("disabled", "disabled"); 
        },
        success: function(cvp) { 
			$('#sonuc').hide().html(cvp).fadeIn("slow");
			$("#iltbtnx").val('TALEBİ GÖNDER').prop("disabled", false);
		}
    });
});
