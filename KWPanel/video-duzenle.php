...
$oku = $db->table('dbo_video_galeri')->where('VidGalID','=',$duzenid)->getRow();
if(!$oku){
	yonver('video-galeri.html');
	exit();
}
...
$isle = $db->table('dbo_video_galeri')->where('VidGalID','=',$duzenid)->update($data);

	if($isle){
		$bilgi = alert('success','Video Düzenlendi','video-galeri.html',3);
		}else{
		$bilgi = alert('danger','Video Düzenlenemedi!','video-galeri.html',3);
	}
...
<link rel="stylesheet" href="../js/summernote/editor.min.css">
	...
	<div class="page-content">
		<header class="page-content-header">
			<div class="container-fluid">
				<div class="tbl">
					<div class="tbl-row">
						<div class="tbl-cell">
							<h3><small class="text-muted">Video Düzenleme</small></h3>
						</div>
		...
		<div class="container-fluid">
        <?=@$bilgi?>
        <form id="form_validation" method="POST" action="video-duzenle.html?id=<?=$oku->VidGalID?>" enctype="multipart/form-data">
			<div class="col-lg-6">
                <section class="card">
                    ...
                    <div class="card-block">
                        <fieldset class="form-group">
                            ...
                            <input type="text" name="vid_gal_adi" value="<?=$oku->vid_gal_adi?>" class="form-control" required>
                        </fieldset>
                        <fieldset class="form-group">
                            ...
                            <select name="vid_gal_kat_id" class="form-control">
								...
								?>
                                <option value="<?=$bkatget->VidGalKat?>" <?php if($bkatget->VidGalKat == $oku->vid_gal_kat_id){ echo 'selected'; }?>><?=$bkatget->vid_kat_adi?></option>
                                <?php } ?>
                            </select>
                        </fieldset>
                       <fieldset class="form-group">
                            <label class="form-label">Video Link (Youtube)</label>
                            <input type="text" name="vid_gal_kod" value="<?=$oku->vid_gal_kod?>" class="form-control" required>
                        </fieldset>
                    </div>
                </section>
            </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-3">
                                <fieldset class="form-group">
                                    ...
                                    <input type="number" name="vid_gal_sira" value="<?=$oku->vid_gal_sira?>" class="form-control" required>
                                </fieldset>
                        	</div>
                        </div>
                    </div>
