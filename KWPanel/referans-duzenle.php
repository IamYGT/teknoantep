...
$oku = $db->table('dbo_referans')->where('RefID','=',$duzenid)->getRow();
if(!$oku){
	yonver('referans-liste.html');
	exit();
}
...
$isle = $db->table('dbo_referans')->where('RefID','=',$duzenid)->update($data);

/*********************************************************************************************/
if(!empty($image->file_dst_name) && !empty($_FILES['ref_logo'])){
	$resup = [ 'ref_logo' => date("Y").'/'.date("m").'/'.$image->file_dst_name ];
	$logo = $db->table('dbo_referans')->where('RefID','=',$oku->RefID)->update($resup);
}
/*********************************************************************************************/


	if($isle){
		$bilgi = alert('success','Referans Düzenlendi','referans-liste.html',3);
		}else{
		$bilgi = alert('danger','Referans Düzenlenemedi!','referans-liste.html',3);
	}
...
<link href="css/separate/vendor/tags_editor.min.css" rel='stylesheet' type='text/css'>
	...
	<div class="page-content">
		...
		<div class="container-fluid">
        <?=@$bilgi?>
        <form id="form_validation" method="POST" action="referans-duzenle.html?id=<?=$oku->RefID?>" enctype="multipart/form-data">
			<div class="col-lg-6">
                <section class="card">
                    ...
                    <div class="card-block">
                        <fieldset class="form-group">
                            ...
                            <input type="text" name="ref_adi" value="<?=$oku->ref_adi?>" class="form-control" required>
                        </fieldset>
                        <fieldset class="form-group">
