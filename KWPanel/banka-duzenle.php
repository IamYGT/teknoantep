...
$oku = $db->table('dbo_banka')->select('*')->where('BankaID','=',$duzenid)->getRow();
if(!$oku){
	yonver('banka-ayarlari.html');
	exit();
}
...
$data = [ 
	...
	$isle = $db->table('dbo_banka')->where('BankaID','=',$oku->BankaID)->update($data);

	if(!empty($image->file_dst_name) && !empty($_FILES['banklogo'])){
			$resup = [ 'banka_logo' => $image->file_dst_name ];
			$logo = $db->table('dbo_banka')->where('BankaID','=',$oku->BankaID)->update($resup);
	}

	if($isle || $logo){
		$bilgi = alert('success','Banka Düzenlendi','banka-ayarlari.html',3);
		}else{
		$bilgi = alert('danger','Banka Düzenlenemedi!','banka-ayarlari.html',3);
	}
...
?>
	...
	<div class="page-content">
		<header class="page-content-header">
			<div class="container-fluid">
				<div class="tbl">
					<div class="tbl-row">
						<div class="tbl-cell">
							<h3><small class="text-muted">Banka Düzenle</small></h3>
						</div>
		...
		<div class="container-fluid">
        <?=@$bilgi?>
        <form id="form_validation" method="POST" action="banka-duzenle.html?id=<?=$oku->BankaID?>" enctype="multipart/form-data">
			<div class="col-lg-6">
                <section class="card">
                    ...
                    <div class="card-block">
                        <fieldset class="form-group">
                            ...
                            <input type="text" class="form-control" value="<?=$oku->banka_adi?>" name="banka_adi" required >
                        </fieldset>
                        <fieldset class="form-group">
                            <label class="form-label">Şube Kodu</label>
                            <input type="text" class="form-control" value="<?=$oku->banka_sb_no?>" name="banka_sube" >
                        </fieldset>
                        <fieldset class="form-group">
                            <label class="form-label">Hesap No</label>
                            <input type="text" class="form-control" value="<?=$oku->banka_h_no?>" name="banka_hesap" >
                        </fieldset>
                        <fieldset class="form-group">
                            <label class="form-label">IBAN/SWIFT/BCKEY</label>
                            <input type="text" class="form-control" value="<?=$oku->banka_iban?>" name="banka_iban" required >
                        </fieldset>
                        <fieldset class="form-group">
                            <label class="form-label">Hesap Sahibi Adı</label>
                            <input type="text" class="form-control" value="<?=$oku->banka_hesapsahibi?>" name="banka_hesapadi" required>
                        </fieldset>
                        <fieldset class="form-group">
