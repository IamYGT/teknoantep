...
$oku = $db->table('dbo_video_galeri_kat')->where('VidGalKat','=',$duzenid)->getRow();
if(!$oku){
	yonver('video-galeri-kat.html');
	exit();
}
...
$isle = $db->table('dbo_video_galeri_kat')->where('VidGalKat','=',$duzenid)->update($data);

	if($isle){
		$bilgi = alert('success','Kategori Düzenlendi','video-galeri-kat.html',3);
		}else{
		$bilgi = alert('danger','Kategori Düzenlenemedi!','video-galeri-kat.html',3);
	}
...
<link href="css/separate/vendor/tags_editor.min.css" rel='stylesheet' type='text/css'>
	...
	<div class="page-content">
		...
		<div class="container-fluid">
        <?=@$bilgi?>
        <form id="form_validation" method="POST" action="video-galeri-kat-duzenle.html?id=<?=$oku->VidGalKat?>" enctype="multipart/form-data">
			<div class="col-lg-6">
                <section class="card">
                    ...
                    <div class="card-block">
                        <fieldset class="form-group">
                            ...
                            <input type="text" name="sc_kat_adi" class="form-control" value="<?=$oku->vid_kat_adi?>" required>
                        </fieldset>
                    </div>
                </section>
            </div>
                ...
                <section class="card">
                    ...
                    <div class="card-block">
                        <fieldset class="form-group">
                            ...
                            <textarea type="text" name="sc_kat_desc" class="form-control" required><?=$oku->vid_kat_desc?></textarea>
                        </fieldset>
                        <fieldset class="form-group">
                            <label class="form-label">Kategori Anahtar Kelime (Virgül İle Ayırın)</label>
                            <textarea id="tags-editor-textarea" type="text" name="sc_kat_meta" class="form-control" rows="5" required><?=$oku->vid_kat_meta?></textarea>
                        </fieldset>
                    </div>
                </section>
            </div>
