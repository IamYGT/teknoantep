...
$oku = $db->table('dbo_resim_galeri')->where('ResGalID','=',$duzenid)->getRow();
if(!$oku){
	yonver('resim-galeri.html');
	exit();
}
...
$isle = $db->table('dbo_resim_galeri')->where('ResGalID','=',$duzenid)->update($data);

	/*********************************************************************************************/
	if(!empty($image->file_dst_name) && !empty($_FILES['resgal_resim'])){
		$resup = [ 'resgal_resim' => date("Y").'/'.date("m").'/'.$image->file_dst_name ];
		$logo = $db->table('dbo_resim_galeri')->where('ResGalID','=',$oku->ResGalID)->update($resup);
	}
	/*********************************************************************************************/
		if(!empty($imagex->file_dst_name) && !empty($_FILES['resgal_resim'])){
		$resupx = [ 'resgal_resim_thumb' => date("Y").'/'.date("m").'/'.$imagex->file_dst_name ];
		$logox = $db->table('dbo_resim_galeri')->where('ResGalID','=',$oku->ResGalID)->update($resupx);
	}
	/*********************************************************************************************/

	if($isle || $resup || $resupx){
		$bilgi = alert('success','Resim Düzenlendi','resim-galeri.html',3);
		}else{
		$bilgi = alert('danger','Resim Düzenlenemedi!','resim-galeri.html',3);
	}
...
<link rel="stylesheet" href="../js/summernote/editor.min.css">
	...
	<div class="page-content">
		<header class="page-content-header">
			<div class="container-fluid">
				<div class="tbl">
					<div class="tbl-row">
						<div class="tbl-cell">
							<h3><small class="text-muted">Resim Düzenleme</small></h3>
						</div>
		...
		<div class="container-fluid">
        <?=@$bilgi?>
        <form id="form_validation" method="POST" action="resim-duzenle.html?id=<?=$oku->ResGalID?>" enctype="multipart/form-data">
			<div class="col-lg-6">
                <section class="card">
                    ...
                    <div class="card-block">
                        <fieldset class="form-group">
                            ...
                            <input type="text" name="resgal_resim_adi" value="<?=$oku->resgal_resim_adi?>" class="form-control" required>
                        </fieldset>
                        <fieldset class="form-group">
                            ...
                            <select name="resgal_kat_id" class="form-control">
								...
								?>
                                <option value="<?=$bkatget->ResGalKatID?>" <?php if($bkatget->ResGalKatID == $oku->resgal_kat_id){ echo 'selected'; }?>><?=$bkatget->rgal_kat_adi?></option>
                                <?php } ?>
                            </select>
                        </fieldset>
                        <fieldset class="form-group">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-3">
                                <fieldset class="form-group">
                                    ...
                                    <input type="number" name="resgal_sira" value="<?=$oku->resgal_sira?>" class="form-control" required>
                                </fieldset>
                        	</div>
                        </div>
                    </div>
