...
$oku = $db->table('dbo_hizmet')
		  ->where('HizmetID','=',$duzenid)
		  ->getRow();
if(!$oku){
	yonver('hizmet-liste.html');
	exit();
}
...
$image = new Upload($_FILES['hizmet_resim']);
	...
	$isle = $db->table('dbo_hizmet')->where('HizmetID','=',$oku->HizmetID)->update($data);

	/*********************************************************************************************/
		if(!empty($image->file_dst_name) && !empty($_FILES['hizmet_resim'])){
		$resupx = [ 'hizmet_resim' => date("Y").'/'.date("m").'/'.$image->file_dst_name ];
		$logox = $db->table('dbo_hizmet')->where('HizmetID','=',$oku->HizmetID)->update($resupx);
	}
	/*********************************************************************************************/

	if($isle || $logox){
		$bilgi = alert('success','Hizmet Düzenlendi','hizmet-liste.html',3);
		}else{
		$bilgi = alert('danger','Hizmet Düzenlenemedi!','hizmet-liste.html',3);
	}
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <fieldset class="form-group">
                                    ...
                                    <input type="text" name="hizmet_adi" value="<?=$oku->hizmet_adi?>" class="form-control" required>
                                </fieldset>
                            </div>
                            <div class="col-md-12">
                                <fieldset class="form-group">
                            ...
                            <div class="col-md-3">
                                <fieldset class="form-group">
                                    ...
                                    <input type="number" name="hizmet_sira" value="<?=$oku->hizmet_sira?>" class="form-control" required>
                                </fieldset>
                            </div>
                        </div>
                    </div>
                ...
                <section class="card">
                    ...
                    <div class="card-block">
                        <fieldset class="form-group">
                            ...
                            <textarea type="text" name="hizmet_aciklama" class="form-control" required><?=$oku->hizmet_aciklama?></textarea>
                        </fieldset>
                        <fieldset class="form-group">
                            <label class="form-label">Hizmet Anahtar Kelime (Virgül İle Ayırın)</label>
                            <textarea id="tags-editor-textarea" type="text" name="hizmet_meta" class="form-control" rows="5" required><?=$oku->hizmet_meta?></textarea>
                        </fieldset>
                    </div>
                </section>
            </div>
                ...
                <section class="card">
                	<div class="card-block">
                     <fieldset class="form-group">
                        ...
                        <textarea id="summernote" name="hizmet_icerik"><?=$oku->hizmet_icerik?></textarea>
                     </fieldset>
                     </div>
                </section>
            </div>
