<?php
@$menu = 'blog-islemleri'; @$page = 'blog-ekle';
...
$data = [ 
		...
		'blog_resim'	=> date("Y").'/'.date("m").'/'.$image->file_dst_name,
		'blog_thumb'	=> date("Y").'/'.date("m").'/'.$imagex->file_dst_name,
		'blog_tarih'	=> $tarih,
		'blog_kat_id'	=> guvenlik($_POST['blog_kat_id'])
		];
		
$isle = $db->table('dbo_blog')->insert($data);

	if($isle){
		$bilgi = alert('success','Blog Eklendi','blog-ekle.html',3);
		}else{
		$bilgi = alert('danger','Blog Eklenemedi!','blog-ekle.html',3);
	}
...
<link rel="stylesheet" href="../js/summernote/editor.min.css">
	...
	<div class="page-content">
		<header class="page-content-header">
			<div class="container-fluid">
				<div class="tbl">
					<div class="tbl-row">
						<div class="tbl-cell">
							<h3><small class="text-muted">Blog Ekleme</small></h3>
						</div>
		...
		<div class="container-fluid">
        <?=@$bilgi?>
        <form id="form_validation" method="POST" action="blog-ekle.html" enctype="multipart/form-data">
			<div class="col-lg-6">
                <section class="card">
                    ...
                    <div class="card-block">
                        ...
                        <fieldset class="form-group">
                            ...
                            <select name="blog_kat_id" class="form-control">
								...
								?>
                                <option value="<?=$bkatget->BlogKatID?>"><?=$bkatget->blog_kat_adi?></option>
                                <?php } ?>
                            </select>
                        </fieldset>
                    </div>
                ...
                <section class="card">
                    ...
                    <div class="card-block">
                        <fieldset class="form-group">
                            ...
                            <textarea type="text" name="blog_desc" class="form-control" required></textarea>
                        </fieldset>
                        <fieldset class="form-group">
                            <label class="form-label">Blog Anahtar Kelime (Virgül İle Ayırın)</label>
                            <textarea id="tags-editor-textarea" type="text" name="blog_meta" class="form-control" rows="5" required></textarea>
                        </fieldset>
                    </div>
                </section>
            </div>
            ...
            <div class="col-lg-12">
                <section class="card">
                	<div class="card-block">
                     <fieldset class="form-group">
                        <label class="form-label">Blog Açıklama</label>
                        <textarea id="summernote" name="blog_icerik"></textarea>
                     </fieldset>
                     </div>
                </section>
            </div>
